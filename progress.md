...
- First quality review found a queued-animation-frame camera race: an intervening new follow/selection movement could be overwritten by the captured old center. Corrected with synchronous size invalidation/restoration and three deferred-callback RED-first regressions. Final parent64Node/141Python pass; real browser expand loops preserve camera and a newer camera movement wins. Final independent quality APPROVED64Node/141Python and200 additional deferred-camera probes; parent actual Leaflet rerun confirms the corrected behavior.
- Fixed spec regression: the single Finish toggle now returns to its original desktop toolbar; real1200×500 Elevation toggle and list passed. Search and Finish-row selections now reveal stats at Info scrollTop0 on phones, preserving view/manual selection. New RED-first coverage61Node green; matchMedia breakpoint synchronization added.
- Spec review blocked short-desktop Elevation Finish-watch access: the toggle’s new parent is hidden outside phone layouts. Fix delegated along with explicit search/Finish selection revealing stats. Parent CDP breakpoint change sometimes failed to deliver resize; explicit resize dispatch restored state, so this is not claimed a physical-browser failure. An initial profile counter used a nonexistent CSS class; counting actual children verified all five authoritative counts.
- Initial tool-wrapped search failed with JSON extra-data parsing; direct search/read succeeded, no source changes from failure. A browser probe accidentally returned the whole Leaflet map and hit an object-chain serialization limit; rerunning with an explicit scalar return succeeded.
