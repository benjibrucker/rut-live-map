...
- **Last recorded check-in:** the selected runner’s actual timing checkpoint, Mountain clock/date and age lead the Info card. A solid recorded-checkpoint marker is separate from the estimated current position on Map/Elevation. These are timing-feed reads, not proof that a runner is still at that checkpoint. Missing clocks or geometry remain explicit; dated/stale histories stay labeled. There is no manual sighting logger.
- **Next checkpoint:** each selected runner’s Info card shows the next timing checkpoint, estimated Mountain arrival and approximate time remaining when the existing server model supports it. This is an unvalidated pace/terrain pilot, not a recorded passage. Missing evidence, stale feeds, finished runners and closed/pre-start races do not produce guesses. If the ETA passes without a new read, the card says **Awaiting checkpoint read** rather than moving the time forward. Snapshot estimates are explicitly not live, show remaining time at capture, and expire after 15 minutes.
- **Checkpoint times & forecasts:** open the selected runner’s single Start → checkpoints → Finish timeline from the next-checkpoint card or bottom controls. Passed checkpoints show actual recorded Mountain clocks and chip elapsed. The next checkpoint is highlighted with its estimated arrival; later checkpoints and Finish show clearly labeled, less-certain forecasts from the same current pace model when supported. Missing earlier reads stay unknown. Once the next expected read is overdue, later forecasts pause rather than sliding times forward or asserting passage. Finished/locationless runners retain recorded history. Organizer start windows/cutoffs never become participant reads; stale/snapshot/partial history stays labeled.
...
- **Red course-direction arrow:** the selected runner’s navigation arrow points forward along the ordered course at their displayed route progress. It is **course direction, not measured heading** or proof of current movement. The source-styled dot/red selection ring and recorded-checkpoint square remain separate. Missing/unsupported route progress, malformed geometry and finished/terminal positions have no arrow; stale/held positions retain their existing source labels.
